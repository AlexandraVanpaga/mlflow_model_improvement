...
ЭТАП 4:
На этом этапе был произведен отбор признаков через Sequential Forward Selection и Sequential Forward Floating Selection, а затем еще через важность признаков по catboost. Признаков было выбрано 12. На отобранных признаках модель повела себя немного лучше: ошибка в реальных единицах на тренировочном сете на 500 000 приблизительно, и r^2 колеблется возле 0.8.
...
Заключение: Модель работает относительно стабильно, можно теоретически добиться лучших результатов, если отбор фичей проводить не на семле из 5000, а на всех данных, также можно Optuna и RandomSearch провести на 30 итерациях, а на CPU это очень медленно. Поменяла какие-то параметры руками, и появились улучшения, так что возможно, что больше итераций Optuna дали бы прирост качества. В целом такая ошибка - это нормально, ведь есть еще такие факторы, как инфраструктура в районе, наличие балкона, ремонт (ремонт вообще может быть отвественным за разницу в цене в пару миллионов). Взвешенная ошибка в процентах - меньше 17%, не так уж плохо.
